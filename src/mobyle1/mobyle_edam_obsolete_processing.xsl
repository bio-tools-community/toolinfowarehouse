    ...
    <xsl:template name="owlClassObsoleteCheck">
         ...
         <xsl:choose>
             <xsl:when test="$edam//owl:Class[@rdf:about=$id]/owl:deprecated">
                 <xsl:comment><xsl:value-of select="$id" /> is deprecated and replaced by <xsl:value-of select="$edam//owl:Class[@rdf:about=$id]/oboInOwl:consider/@rdf:resource" /></xsl:comment>
                 <xsl:value-of select="$edam//owl:Class[@rdf:about=$id]/oboInOwl:consider/@rdf:resource" />
             </xsl:when>
             <xsl:otherwise>
