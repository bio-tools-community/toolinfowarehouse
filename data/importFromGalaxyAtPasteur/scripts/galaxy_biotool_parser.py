...
def build_metadata_one(tool_meta_data):
    """
      builds general_dict
      @param: tool_meta_data for one tool extracted from galaxy
    """
    gen_dict = {k: tool_meta_data[k] for k in (u'name', u'id', u'version', u'description')}
    gen_dict[u'collectionName'] = 'GalaxyAtPasteur'
...
def build_fonction_dict(tool_meta_data):
    ...
    for input in inputs_complete:
        # ici mettre un try catch pour gerer 'extensions' et non 'format'"
        elem = {i: input[i] for i in (u'format', u'type', u'label', u'name')}
        inputs.append(elem)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tool_dir", help="directory to store the tool json (needs to be created before running the script")
